...
\begin{column}{\colwidth}
  ...
  \begin{block}{Methods} \label{methods}
	...
	To address \#1, the three metrics are calculated for each GLAHF subregion and ice season and then Mann-Kendall monotonic trend test\supercite{Mann1945} is applied to check for statistical significance. Additionally correlations between trend magnitude and morphometrics are identified. To address \#2, station data\supercite{Menne2012,Smith2011} is aggregated for each subregion based on a 20 KM radius and correlations between ice metrics and monthly mean (or max for wind) meteorological surface variables are identified.
...
\begin{figure}
 ...
 \caption{Linear trends for each metric. Open (solid) dots indicate 95\% (99\%) statistical significance. Columns correspond to rows in fig \ref{fig:dur}. }
  \label{fig:trends}
\end{figure}
			Season duration decrease is visible in some regions but not apparent in others (see fig. \ref{fig:dur}).  Regions that show decrease are often offshore and deep (WSU, CSU, ESU, NHU, EHU, CHU, WON, CON). This may be due to warmer subsurface temps\supercite{Cannon2024, Anderson2021} during later years providing heat to surface waters and preventing freezing, whereas shallow regions may be able to consistently release all heat from the water column. 
	\vspace{-5ex}
\begin{figure} 
...
%	\centering	\large \textbf{\underline{Part 2: Correlation with Met Stations}} \normalfont
				\begin{itemize}
				\item depth and area show significant influence on AMIC and duration change
				\item latitude showed the most influence on seasonal average ice change 
				\end{itemize}

	
	\begin{figure} 
		\includegraphics[width=0.8\textwidth]{../figures/cor/SB.png}
		\includegraphics[width=0.8\textwidth]{../figures/cor/NHU.png}
	  ...
	  \begin{itemize}
		  ...
		  \item Strong inverse correlation with air temp, particularly during Jan and Feb. 
		  \item Precip showed minimal correlation during any month/region while wind and cloud cover had moderate correlations. 
		  \item Since wind contributes to surface mixing which ultimately cools the water column and clouds primarily reflect solar radiation, we expect direct correlations with ice cover: True for Saginaw Bay but not Northern Huron. 
		  \item Correlations varied considerably between subregions but $\textit{within}$ a given subregion, the correlations were fairly consistent between AMIC, JFM and ice duration (not shown).
		\item A number of post ice season correlations were identified: provides valuable insight into physical processes but are not meaningful for prediction. 
\end{itemize}
\begin{block}{Summary}\label{summary}
		...
		\begin{itemize}
			...
			\item Wind, cloud cover correlations with ice cover varied from region to region
			\item Future work: predictive model development: step-wise linear regression or machine-learning
		\end{itemize}
		\vspace{-5ex}
		\raggedleft
		References: \includegraphics[width=.125\textwidth]{../figures/bib_qr.png}
  \end{block}
